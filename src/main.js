...
const resumeData = {
  name: "Ahn Sungjin",
  title: "Generative AI Project Manager",
  description: "Driving practical innovation by integrating large-scale IT/DX project leadership with cutting-edge Generative AI technologies (vLLM, Multi-Agent, RAG, MCP) to solve complex enterprise challenges.",
  coreRoles: [
    {
      title: "1. Gen AI 서비스 아키텍처 설계 및 시스템 통합 총괄",
      desc: "",
      bullets: [
        "엔터프라이즈 환경(On-Premise/Cloud)을 고려한 Gen AI 서비스 아키텍처 수립 및 기술 이행 전략 리딩",
        "K8s 기반 인프라 환경의 기술적 제약사항 조율 및 생성형 AI 파이프라인 전 과정의 이슈 해결 주도",
        "레거시 및 타 시스템과의 연계를 고려한 시스템 통합 아키텍처 설계 및 검토",
        "정형/비정형 데이터 수집부터 RAG 기반 서비스 오픈까지 생성형 AI 데이터 파이프라인 전 과정 총괄"
      ]
    },
    {
      title: "2. Agentic AI 워크플로우 기획 및 프로덕트 생애주기(E2E) 리딩",
      desc: "",
      bullets: [
        "제품 목표 설정부터 요구사항 분석, 개발, 출시, 운영까지 PM으로서 AI 프로덕트 생애주기 직접 리딩",
        "LLM 및 Agentic AI에 대한 기술적 이해를 바탕으로 비즈니스 목적에 부합하는 Multi-Agent 워크플로우 기획",
        "단순 챗봇을 넘어선 목적별 서브 에이전트 간 협력 및 Task 자동화 파이프라인 구축 리딩",
        "특정 도메인(금융 등)의 이해를 바탕으로 실질적인 업무 생산성을 제고하는 AI 비즈니스 모델 발굴 및 적용"
      ]
    },
    {
      title: "3. AI 기반 업무 생산성 혁신 및 Full-Cycle 프로토타이핑",
      desc: "",
      bullets: [
        "AI를 활용한 실질적인 업무 생산성 제고에 지속적인 관심을 두고, 바이브 코딩 등 다양한 방법론을 실무에 적극 도입 및 시도",
        "AI 바이브 코딩을 활용하여 기획 단계부터 사내 프로젝트 관리(WBS) 시스템을 직접 기획, 구축 및 배포한 경험 보유",
        "제안 단계 및 신규 서비스 기획 시, PoC 데모 페이지 제작부터 서버 구축, 배포까지 전 과정을 직접 수행하여 제품 방향성 및 인사이트 빠른 검증",
        "불확실성이 높은 AI 비즈니스 환경에서 프로토타이핑 툴을 활용한 빠른 아이디어 실체화로 의사결정 가속화"
      ]
    },
    {
      title: "4. LLM & Agentic AI 기반 시스템 구축 및 인프라 아키텍처 총괄",
      desc: "",
      bullets: [
        "LLM, Agentic AI 등 최신 기술 이해도를 바탕으로 한 AI 모델 기반 시스템 구축 전 주기 리딩",
        "RAG 파이프라인 설계 및 Tool Calling, 워크플로우 오케스트레이션을 연계한 복합 AI 서비스 구현",
        "대규모 AI 학습·추론을 위한 GPU 인프라 아키텍처 설계 및 vLLM 서빙 최적화를 통한 AI HPC급 환경 구축",
        "성능, 비용, 지연시간을 종합적으로 고려하여 대용량 트래픽 처리가 가능한 인프라 환경 구축 경험 보유"
      ]
    }
  ]
};

// Portfolio Data (Static — edit directly in code)
const portfolioData = [
  {
    id: "proj1",
    title: "S 증권사 생성형 AI 리서치 포털 구축 (Gen AI Research Assistant)",
    role: "총괄 AI PM (Architecture Design & Delivery Lead)",
    period: "2025.07 ~ 진행 중 | GenON AI",
  ...
  {
    id: "proj3",
    title: "WBS 기반 사내 통합 프로젝트 관리 시스템 구축",
    role: "기획 및 풀스택 프론트/백엔드 완전 개발",
    period: "2025.12 ~ 2026.03 | 사내 프로젝트",
    tags: ["Vibe Coding", "React 19", "Node.js", "PostgreSQL", "Docker", "풀스택"],
    coverColor: "linear-gradient(135deg, #0f172a 0%, #0a2e1a 50%, #1a3d22 100%)",
    coverIcon: "⚙️",
    summary: "AI 스타트업 내부에서 사용하는 프로젝트 WBS(Work Breakdown Structure) 관리 시스템과 AI PM 온보딩 가이드 대시보드를 직접 기획하고 Vibe Coding 방식으로 개발하여 실무 적용한 웹 애플리케이션. 기존 엔셀 기반으로 관리하던 프로젝트 일정 및 산출물 추적 업무를 웹 애플리케이션으로 전환.",
    challenge: "다수의 AI 프로젝트가 동시에 진행되는 환경에서 각 PM별 일정 및 산출물 관리 방식이 파편화되어 전사 현황 파악과 리소스 배분이 어려웠습니다. 엔터프라이즈급 툴은 실시간성과 팀 매니지먼트 요구에 반응하기 어렵고, 신규 PM 온보딩에도 많은 시간이 소요되었습니다.",
    solution: "Vibe Coding(코딩 어시스턴트와의 긴밀한 AI 협업)으로 기획부터 코드 완성까지 직접 주도하여 제작했습니다. 프론트(React 19 SPA)부터 백엔드(Express REST API), PostgreSQL DB 설계, Docker Compose 기반 배포까지 전 영역을 단독 담당하였습니다. 자체 집필한 AI PM 방법론 가이드를 시스템 내에 탑재하여 지식 자산화를 완성했습니다.",
    results: [
      "사내 실제 도입: 팀 내 프로젝트 일정 관리 도구로 실무 적용, 엑셀 기반 업무 대체",
      "풀스택 단독 개발: React SPA + REST API + PostgreSQL DB + Docker/Nginx 인프라 전 영역 직접 담당",
      "Vibe Coding 방식으로 기획·요구사항 정의부터 코드 완성까지 AI와 협업하여 주도적으로 전체 사이클 경험",
      "컨테이너 기반 배포: Docker Compose로 DB·백엔드·프론트엔드 3개 서비스를 한 번에 구동, 사내 서버에 직접 배포 운영"
    ],
    techStack: ["React 19 (Vite)", "React Router DOM v7", "Node.js / Express.js", "PostgreSQL 16", "Docker + Docker Compose", "Nginx", "JWT + Google OAuth 2.0", "XLSX / jsPDF"],
    architecture: `
      <h4>시스템 아키텍처</h4>
      <table>
  ...
  {
    id: "proj4",
    title: "ARIBOX & GENGA — AI 기반 3D 컨텐츠 자동화 플랫폼",
    role: "프로젝트 총괄 PM 및 AI 기반 플랫폼 전략 수립",
    period: "2022.01 ~ 2024.09",
    tags: ["Photogrammetry", "Vision AI", "Gaussian Splatting", "Deep Learning", "Generative AI"],
    coverColor: "linear-gradient(135deg, #1a0a2e 0%, #2d1b4e 50%, #1a1a3e 100%)",
    coverIcon: "🧊",
    summary: "딥러닝, 생성형 AI, Gaussian Splatting 기술을 융합한 3D 콘텐츠 자동화 플랫폼 구축. 반사, 투명, 금속 등 난이도 높은 재질의 객체를 사진만으로 정확하게 3D 모델링하고 사실적인 텍스처를 자동 매핑하는 핵심 기술을 개발. ARIBOX는 Photogrammetry 기반 3D 오브젝트 생성 자동화를, GENGA AI는 Gaussian Splatting 기반 고품질 실시간 렌더링을 담당하며, 두 기술의 결합으로 기존 수작업 3D 제작 대비 획기적인 시간·비용 절감을 실현.",
    challenge: "1) 반사·투명·금속 등 다양한 재질의 객체를 사진 기반으로 정확하게 3D 모델링하고 사실적인 텍스처를 자동 매핑하는 기술적 난제. 2) Photogrammetry 과정에서 발생하는 불완전하거나 노이즈가 포함된 3D 스캔 데이터를 AI를 통해 자동 복원하여 완성도를 높이는 핵심 기술 개발. 3) 기존 Photogrammetry 방식의 한계를 넘어, 적은 수의 이미지로도 고품질 3D 모델 재구성이 가능한 AI 기반 알고리즘 적용. 4) 실시간 고품질 3D 렌더링이 가능하면서도 다양한 플랫폼(웹, 모바일, 키오스크)에 적용 가능한 경량화 렌더링 솔루션 확보.",
    solution: "ARIBOX 기술로 사진 기반 3D 모델 자동 생성 파이프라인을 구축하여 촬영 → 3D 복원 → 텍스처 매핑 → 후처리까지 전 과정을 자동화했습니다. 반사/금속 재질의 노이즈 제거를 위해 Vision AI 기반 후처리 모듈을 도입하고, GENGA AI(Gaussian Splatting 기반)를 적용하여 기존 렌더링 방식 대비 대폭 향상된 속도의 실시간 렌더링을 구현했습니다. 이를 기반으로 '스크립트 기반 3D 뷰어', '3D 랜딩 페이지 제작 도구' 등 B2B 서비스로 상용화에 성공했습니다.",
    results: [
      "3D 콘텐츠 제작 기간 75% 단축 및 비용 60% 절감",
      "네이버 브랜드 스토어 공식 3D 솔루션 파트너 선정",
      "호카, 언더아머, 로보락 등 30개 이상 국내외 유명 브랜드 서비스 제공"
    ],
    techStack: ["Photogrammetry", "Vision AI", "Gaussian Splatting", "Deep Learning", "Generative AI"],
    architecture: `
  ...
  {
    id: "proj5",
    title: "건돌이닷컴 — AI 기반 소셜 미디어 통합 분석 SaaS 플랫폼",
    role: "프로젝트 총괄 PM 및 AI 기반 서비스 전략 수립",
    period: "2019.08 ~ 2021.02",
    tags: ["SNS API", "NLP", "AWS Cloud", "RPA", "SaaS"],
    coverColor: "linear-gradient(135deg, #0a2e1a 0%, #1a3d2a 50%, #0d1f14 100%)",
    coverIcon: "📊",
    summary: "인스타그램, 페이스북, 유튜브 등 다중 SNS 채널 데이터를 통합 분석하고 NLP를 활용해 마케팅 인사이트를 제공하는 SaaS 플랫폼 기획 및 상용화. 소셜 미디어 API 연동을 통한 데이터 수집, 비정형 텍스트의 NLP 분석(해시태그 트렌드, 연관 키워드, 감성 분석), 플랫폼별 지표 표준화, RPA 기반 운영 자동화(멀티 플랫폼 예약 발행, 통합 댓글 관리)를 구현하여 정기 구독 기반 유료 서비스로 상용화.",
    challenge: "1) 소셜 미디어 API 연동 시 플랫폼별 데이터 구조·지표 체계가 상이하여 통합 분석을 위한 데이터 표준화 설계가 필요. 2) 비정형 텍스트 데이터(한국어 해시태그, 댓글, 캡션)의 NLP 분석 정확도 확보 — 해시태그 트렌드 분석, 연관 키워드 추출, 감성 분석(긍정/부정/중립) 파이프라인 구축. 3) RPA를 통한 반복 업무 자동화(멀티 플랫폼 예약 발행, 통합 댓글 관리, 정기 리포트 생성)로 운영 효율 극대화. 4) 멀티 테넌시 SaaS 아키텍처에서 고객별 데이터 격리와 안정적 서비스 운영.",
    solution: "NLP 분석 및 운영 자동화를 포함한 핵심 서비스 컨셉을 정의하고, AWS 클라우드 기반 확장 가능한 아키텍처를 설계했습니다. NLP 엔진을 통해 해시태그 트렌드 분석·연관 키워드 자동 추출·감성 분류 기능을 구현하고, RPA 모듈로 멀티 플랫폼 예약 발행·통합 댓글 관리·주간 리포트 자동 생성·메일 발송까지 운영 전반을 자동화했습니다. 인스타그램(Meta) 공식 미디어 파트너 API 승인을 획득하여 공식 데이터 접근 권한을 확보했습니다.",
    results: [
      "2,500개 이상의 유료 계정 확보 (정기 구독 SaaS)",
      "60,000개 이상의 소셜 데이터 분석 처리",
      "운영 리소스 50% 이상 절감 (RPA 자동화)",
      "인스타그램(Meta) 공식 미디어 파트너 API 승인 획득"
    ],
    techStack: ["SNS API (Meta Graph API, YouTube Data API)", "NLP (감성 분석, 키워드 추출)", "AWS Cloud", "RPA"],
    architecture: `
      <h4 style="color: var(--accent-primary); margin: 1.5rem 0 0.8rem;">주요 역할</h4>
      <ul style="padding-left: 1.5rem; margin-bottom: 1.5rem;">
  ...
  {
    id: "proj6",
    title: "현대/기아 360° 3D Configurator Platform 구축",
    role: "프로젝트 총괄 PM 및 유니티 엔진 기반 시스템 기획",
    period: "2020.02 ~ 2023.09",
    tags: ["Unity", "Real-time 3D", "Web API", "글로벌 배포", "다국어", "3D 경량화"],
    coverColor: "linear-gradient(135deg, #1a1a2e 0%, #16213e 50%, #0f3460 100%)",
    coverIcon: "🚗",
    summary: "전 세계 현대/기아 자동차 매장에서 사용되는 실시간 3D 차량 정보 제공 플랫폼을 유니티 엔진 기반으로 구축. API 연동을 통해 차종별 최신 사양·가격 정보를 실시간 업데이트하고, 색상·휠·인테리어 등 복잡한 옵션 조합을 인터랙티브한 360° 3D로 시각화. 대용량 3D 모델 데이터 경량화 가이드라인을 수립하고 다국어·다기기 최적화 UX를 기획하여 전 세계 20여 개국 딜러십에 성공적으로 배포.",
    challenge: "1) 대규모 3D 에셋(차량 외장, 내장, 옵션별 파츠) 최적화 및 웹/키오스크 환경에서의 고품질 실시간 렌더링 성능 확보. 2) 글로벌 20개국 동시 서비스를 위한 서버 인프라 구축 및 다국어·다기기 대응. 3) 수백 가지 차량 옵션 조합(색상, 휠, 인테리어, 트림 등)에 따른 3D 모델 동적 교체 로직 설계 — 옵션 간 상호 종속성 및 국가별 사양 차이 반영.",
    solution: "유니티 엔진 기반 실시간 3D 렌더링 시스템을 기획하고, API 연동을 통한 차종별 사양·가격 실시간 업데이트 체계를 설계했습니다. 대용량 3D 모델 데이터 경량화 가이드라인을 수립하여 로딩 속도와 렌더링 성능을 최적화하고, 옵션 조합별 3D 에셋 모듈화(색상, 휠, 인테리어 독립 교체) 구조를 설계했습니다. 다국어 지원(20개국) 및 기기별(태블릿, 키오스크, PC) 최적화 UX를 기획하여 글로벌 딜러십에 안정적으로 배포했습니다.",
    results: [
      "전 세계 20여 개국 딜러십 성공적 배포 및 안정 운영",
      "제작 비용 약 56% 절감 실현을 통한 3년간 장기 수주 계약 확보"
    ],
    techStack: ["Unity Engine", "Real-time 3D Rendering", "Web API", "글로벌 CDN", "다국어(i18n)"],
    architecture: `
      <h4 style="color: var(--accent-primary); margin: 1.5rem 0 0.8rem;">주요 역할</h4>
      <ul style="padding-left: 1.5rem; margin-bottom: 1.5rem;">
  ...
  {
    id: "proj7",
    title: "현대글로비스 AR 기반 차량 관리 시스템 구축",
    role: "지능형 차량 관리 시스템 기획 및 PM",
    period: "2019.06 ~ 2019.11",
    tags: ["AR", "Google Vision API", "OCR", "Smart Mirror", "Mobile App"],
    coverColor: "linear-gradient(135deg, #2d1b0e 0%, #3d2b1e 50%, #1a0f05 100%)",
    coverIcon: "🔍",
    summary: "광학 문자 인식(OCR) 및 증강 현실(AR) 기술을 활용한 지능형 차량 관리 플랫폼 구축. Google Vision API 기반 OCR 엔진으로 번호판을 자동 인식하고, AR 기술로 야적장 내 차량 정보를 실시간 시각화하여 기존 수작업 점검 프로세스를 디지털화. 스마트 미러(고정형)와 모바일 앱(이동형) 이중 디바이스 체계로 현장 운영 효율성을 극대화.",
    challenge: "1) 광학 문자 인식(OCR) 기반 번호판 자동 인식의 정확도 확보 — 야적장 환경(야외 조명 변화, 오염·각도 편차)에서도 안정적인 번호판 인식률 달성. 2) AR 기술을 활용한 실시간 차량 정보 시각화 — 번호판 인식 결과와 차량 DB를 연동하여 AR 오버레이로 차량 상태·이력 정보를 즉시 표시. 3) 현장 작업자의 사용성 최적화 — 장갑 착용, 야외 밝기, 한 손 조작 등 현장 제약 조건을 반영한 UX 설계.",
    solution: "Google Vision API를 활용한 OCR 엔진으로 번호판 인식 정확도를 고도화하고, 인식 결과를 차량 DB와 실시간 연동하여 AR 오버레이로 차량 정보를 시각화하는 시스템을 설계했습니다. 스마트 미러(야적장 출입구 고정 설치형)와 모바일 앱(현장 이동형)의 이중 디바이스 체계를 구축하여 다양한 운영 시나리오에 대응했습니다. 현장 PoC를 통해 야외 환경에서의 인식률을 검증하고 사용자 피드백을 반영하여 UI를 최적화했습니다.",
    results: [
      "경매 차량 관리 효율성 향상 및 인적 오류 최소화",
      "수작업 점검 → 디지털 데이터 기반 관리 체계 전환",
      "산업 현장 내 AR/AI 결합 솔루션 성공 사례 구축"
    ],
    techStack: ["AR (Augmented Reality)", "Google Vision API (OCR)", "Smart Mirror", "Mobile App"],
    architecture: `
      <h4 style="color: var(--accent-primary); margin: 1.5rem 0 0.8rem;">주요 역할</h4>
      <ul style="padding-left: 1.5rem; margin-bottom: 1.5rem;">
  ...
  {
    id: "proj8",
    title: "기아 남양연구소 가상 디자인 품평장 시스템 구축",
    role: "국내 최초 무선 방식 VR 품평 시스템 기획 및 개발 총괄",
    period: "2020.05 ~ 2020.11",
    tags: ["VR", "Antilatency Tracking", "다중 접속", "무선 스트리밍", "디자인 품평", "혼합현실"],
    coverColor: "linear-gradient(135deg, #0e2d1b 0%, #1e3d2b 50%, #051a0f 100%)",
    coverIcon: "🥽",
    summary: "국내 최초로 Antilatency를 이용한 무선 방식의 다중 접속 VR 디자인 품평 시스템을 기아 남양연구소에 구축. 기존 유선 VR의 이동 범위 제한을 무선 트래킹으로 해결하고, 다중 사용자가 동일한 가상 공간에서 실물 스케일의 차량 디자인을 실시간으로 검토. 실제 클레이 모델과 VR 디자인을 실시간으로 비교 분석할 수 있는 혼합현실 환경을 구현하여 디자인 의사결정 프로세스를 혁신.",
    challenge: "1) 다중접속 VR 품평 시스템 구축 — 다수의 디자이너/임원이 동시에 접속하여 실물 스케일의 차량 디자인을 함께 검토할 수 있는 실시간 동기화 환경 구현. 2) 무선 대공간 트래킹 — 기존 유선 VR(제한된 이동 범위)의 한계를 넘어, 실물 크기 차량 주변을 자유롭게 이동하며 관찰 가능한 대공간 무선 트래킹 인프라 설계. 3) 혼합현실 비교 품평 — 실제 차량(클레이 모델)과 AR로 구현된 디자인 시안을 실시간으로 비교 분석할 수 있는 환경 구현.",
    solution: "Antilatency 기반 Tracking Area를 설계하여 무선 환경에서의 정밀 위치 추적을 구현했습니다. 다중 접속 동기화 시스템을 구축하여 여러 사용자가 동일한 가상 공간에서 실시간으로 차량 디자인을 함께 검토할 수 있도록 했습니다. 실제 클레이 모델 위에 VR 디자인을 오버레이하는 혼합현실 비교 품평 환경을 조성하고, 디자인 의사결정 프로세스 최적화 컨설팅을 수행했습니다.",
    results: [
      "국내 최초 다중접속 무선 VR 품평 시스템 성공적 구축",
      "디자인 의사결정 시간 기존 대비 60% 단축"
    ],
    techStack: ["VR", "Antilatency Tracking", "Real-time Wireless Streaming", "혼합현실(MR)"],
    architecture: `
      <h4 style="color: var(--accent-primary); margin: 1.5rem 0 0.8rem;">주요 역할</h4>
      <ul style="padding-left: 1.5rem; margin-bottom: 1.5rem;">
...
function renderHome() {
  const coreRolesHtml = resumeData.coreRoles.map(role => {
    const bulletList = role.bullets
      ? `<ul>${role.bullets.map(b => `<li style="color: var(--text-muted); font-size: 0.95rem; line-height: 1.6; margin-bottom: 0.4rem;">${b}</li>`).join('')}</ul>`
      : '';

    return `
      <div class="skill-category">
        <h3>${role.title}</h3>
        ${role.desc ? `<p style="color: var(--text-muted); font-size: 0.95rem; line-height: 1.6; margin-bottom: 1rem;">${role.desc}</p>` : ''}
        ${bulletList}
      </div>
    `;
  }).join('');
    ...
    <section class="hero panel">
      <div class="hero-layout">
        <div class="hero-text">
          ...
          <h2 class="hero-title">${resumeData.title}</h2>
...
function renderPortfolio() {
  const cardsHtml = portfolioData.map(proj => {
    const tagsHtml = proj.tags.map(t => `<span class="port-tag">${t}</span>`).join('');
    return `
      <article class="port-card" onclick="openPortfolioDetail('${proj.id}')">
        <div class="port-card-body">
          <div class="port-tags">${tagsHtml}</div>
          <h3 class="port-card-title">${proj.title}</h3>
          <p class="port-card-role">${proj.role} &nbsp;|&nbsp; ${proj.period}</p>
          <p class="port-card-summary">${proj.summary}</p>
...
function renderPortfolioDetail(projId) {
  const proj = portfolioData.find(p => p.id === projId);
  if (!proj) return renderPortfolio();

  const tagsHtml = proj.tags.map(t => `<span class="port-tag">${t}</span>`).join('');
  const resultsHtml = proj.results.map(r => `<li>${r}</li>`).join('');
  const techHtml = proj.techStack.map(t => `<code class="tech-chip">${t}</code>`).join(' ');
    ...
    <section class="portfolio panel">
      ...
      <div class="detail-header">
        ...
        <div class="detail-meta-row">
          <span class="detail-meta-item"><strong>역할</strong> ${proj.role}</span>
          <span class="detail-meta-sep">·</span>
          <span class="detail-meta-item"><strong>기간</strong> ${proj.period}</span>
        </div>
        <div class="port-tags" style="margin-top: 0.8rem;">${tagsHtml}</div>
      </div>
      ...
      <article class="detail-article">
        ...
        ${proj.summary ? `<h2>프로젝트 개요</h2><p>${proj.summary}</p>` : ''}
        ${proj.challenge ? `<h2>도전 과제 (Challenge)</h2><p>${proj.challenge}</p>` : ''}
        ${proj.solution ? `<h2>접근 방식 (Solution &amp; Architecture)</h2><p>${proj.solution}</p>` : ''}
        
        ${proj.architecture}

        ${proj.results && proj.results.length > 0 ? `<h2>주요 성과 (Results)</h2><ul class="detail-results">${resultsHtml}</ul>` : ''}
        ${proj.techStack && proj.techStack.length > 0 ? `<h2>기술 스택</h2><div class="tech-stack">${techHtml}</div>` : ''}
...
window.openPortfolioDetail = function (projId) {
  updateNavActive('#portfolio');
  mainContent.innerHTML = '';
  setTimeout(() => {
...
window.navigateTo = function (hash) {
  ...
  // hashchange won't fire if already on the same hash, so force a re-render
  handleRoute();
}

window.printFullPortfolio = function () {
  // Save original state
  const originalY = window.scrollY;
  const originalHtml = mainContent.innerHTML;

  // 1. Generate Cover Page HTML — exact A4 height, centered
  const coverHtml = `
    <div class="pdf-cover" style="height: 277mm; display: flex; flex-direction: column; justify-content: center; align-items: center; text-align: center; padding: 20px; box-sizing: border-box; overflow: hidden;">
      ...
      <h2 style="font-size: 2.2rem; color: #ffffff; margin-bottom: 0.4rem; font-weight: 600;">${resumeData.name}</h2>
      <h3 style="font-size: 1.3rem; color: #a1a1aa; font-weight: 400; letter-spacing: 1px;">AI Project Manager</h3>
    </div>
    <div class="html2pdf__page-break"></div>
  `;

  // 2. Generate Core Competencies Page HTML — fills one page nicely
  const coreRolesHtml = resumeData.coreRoles.map(role => {
    const bulletList = role.bullets
      ? `<ul style="margin: 0 0 0 1rem; padding: 0; list-style-type: disc;">${role.bullets.map(b => `<li style="color: #ffffff; font-size: 0.88rem; line-height: 1.65; margin-bottom: 0.3rem;">${b}</li>`).join('')}</ul>`
      : '';
    return `
      <div style="background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.1); border-radius: 8px; padding: 14px 16px; break-inside: avoid;">
        <h4 style="font-size: 0.95rem; color: #38bdf8; margin: 0 0 0.6rem 0; padding-bottom: 0.4rem; border-bottom: 1px solid rgba(56,189,248,0.2);">${role.title}</h4>
        ${bulletList}
      </div>
    `;
  }).join('');
  ...
  // 3. Generate combined HTML for all projects with page breaks
  const allDetailsHtml = portfolioData.map((proj, index) => {
    let detailHtml = renderPortfolioDetail(proj.id);
    // Remove the back button from the print version
    detailHtml = detailHtml.replace(/<button class="back-btn".*?<\/button>/, '');

    // Add page break after each project except the last
    if (index < portfolioData.length - 1) {
      detailHtml += '<div class="html2pdf__page-break"></div>';
    }
    return detailHtml;
  }).join('');

  // Change button text to show progress (grab before replacing content)
  const btn = document.querySelector('button[onclick="window.printFullPortfolio()"]');
  const originalBtnHtml = btn ? btn.innerHTML : '';
  if (btn) {
    btn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> 이미지 로딩 중...';
    btn.disabled = true;
  }

  // Wrap everything in a PDF container — dark theme + white body text + accent headers
  const printStyles = `
    <style>
  ...
  // Assemble the final HTML to be rendered by html2canvas
  mainContent.innerHTML = printStyles + '<div class="pdf-wrapper">' + coverHtml + compPageHtml + allDetailsHtml + '</div>';
  window.scrollTo(0, 0);

  // Configure html2pdf options — dark background preserved, margin 0 to eliminate white borders
  const opt = {
    margin: 0,
    filename: 'Ahn_Sungjin_Portfolio.pdf',
    image: { type: 'jpeg', quality: 0.98 },
    html2canvas: { scale: 2, useCORS: true, logging: false, backgroundColor: '#0a0a0a' },
    jsPDF: { unit: 'mm', format: 'a4', orientation: 'portrait' }
  };

  // Wait for all images inside mainContent to fully load to prevent content cut-off
  const images = Array.from(mainContent.querySelectorAll('img'));
  let loadedCount = 0;

  const finishPDF = () => {
    if (btn) btn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> PDF 변환 중...';

    // Slight pause to ensure browser repaints DOM completely before canvas capture
    setTimeout(() => {
      // Generate PDF from the now-visible mainContent
      window.html2pdf().set(opt).from(mainContent).save().then(() => {
  ...
  if (images.length === 0) {
    ...
    images.forEach(img => {
      if (img.complete) {
        loadedCount++;
        if (loadedCount === images.length) finishPDF();
      } else {
        img.onload = () => {
          loadedCount++;
          if (loadedCount === images.length) finishPDF();
